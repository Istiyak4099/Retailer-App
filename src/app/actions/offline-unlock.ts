...
/**
 * Generate the 8-character alphanumeric response code for a given challenge.
 * 
 * Algorithm: HMAC-SHA256(HMAC_SECRET, challenge) → first 8 bytes mapping → 8 chars
 * This matches the updated device-side OfflineUnlockKeyGenerator exactly.
 *
 * @param challenge - The challenge code shown on the device
 * @returns 8-character alphanumeric response string
 */
export async function generateOfflineUnlockResponse(challenge: string): Promise<string> {
